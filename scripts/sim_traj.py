...
def simulate(
    ...
    for run_idx in range(n_runs):
        ...
        # Optional trajectory visualizer (sim-level; works for any controller)
        viz = TrajectoryVisualizer(config=config, initial_obs=obs, title=controller_cls.__name__) if v else None
        ...
        while True:
            ...
            if viz is not None:
                viz.update(obs=obs, action=action, info=info)
        ...
        if viz is not None:
            viz.finalize(file_prefix=controller_name)
